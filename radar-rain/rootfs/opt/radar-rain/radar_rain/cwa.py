...
class CwaRadarClient:
    def __init__(self, timeout: float = 45.0):
        ...
        self._frame_cache: dict[int, RadarFrame] = {}
    ...
    async def fetch_frames(self, count: int = 3) -> list[RadarFrame]:
        ...
        for slot in slots:
            frame = self._frame_cache.get(slot)
            if frame is None:
                response = await self._client.get(frame_url(slot))
                if response.status_code == 404:
                    continue
                response.raise_for_status()
                frame = parse_xml(response.content, datetime.fromtimestamp(slot, UTC))
                self._frame_cache[slot] = frame
            frames.append(frame)
            if len(frames) >= count:
                break
        if len(frames) < 2:
            raise RuntimeError("fewer than two recent CWA radar frames are available")
        keep_slots = set(slots[: count + 1])
        self._frame_cache = {
            slot: frame for slot, frame in self._frame_cache.items() if slot in keep_slots
        }
